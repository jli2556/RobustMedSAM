...
class TrainDataset(Dataset):
    def __init__(self, opt, mode=None, training_size=512):
        ...
        self.transform_list = ['clear', 'gauss_noise', 'gaussian_blur', 'contrast', 'brightness',
        ...
        self.degraded_count_path = os.path.join(self.root_dir, 'gauss_noise')
        self.degraded_count_list = sorted(glob(os.path.join(self.degraded_count_path, '*jpg')))
        print('Number of training images: {}'.format(min(len(self.clear_image_list), len(self.degraded_count_list))))
    ...
    def __len__(self):
        return min(len(self.clear_image_list), len(self.degraded_count_list))
